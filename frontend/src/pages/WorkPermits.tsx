...
const WorkPermits = () => {
  ...
  const exportSelectedToExcel = () => {
    if (selectedPermits.length === 0) {
      toast({
        title: "تحذير",
        description: "الرجاء اختيار التصاريح المراد تصديرها",
        variant: "destructive",
      });
      return;
    }

    const selectedData = filteredPermits
      .filter(permit => selectedPermits.includes(permit.id))
      .map((permit, index) => ({
        'م': index + 1,
        'SR': permit.sr,
        'نوع التصريح': permit.type,
        ...
        'تاريخ النهاية': permit.endDate,
        'الحالة': permit.status,
        'التفاصيل': permit.details
      }));

    const ws = XLSX.utils.json_to_sheet(selectedData, {
      header: ['م', 'SR', 'نوع التصريح', 'القرية', 'الوحدة/المتجر', 'أنشئ بواسطة', 'اسم الموظف', 'تاريخ البداية', 'تاريخ النهاية', 'الحالة', 'التفاصيل']
    });
    ...
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'تصاريح العمل');
    XLSX.writeFile(wb, `تصاريح_العمل_المحددة_${format(new Date(), 'yyyy-MM-dd')}.xlsx`);
    
    toast({
      title: "تم التصدير",
      description: "تم تصدير التصاريح المحددة بنجاح",
    });
  };
        ...
        <SidebarInset className="overflow-auto">
          ...
          <main className="flex-1 p-6">
            ...
            <div className="flex gap-4 mb-6">
              ...
              <Button onClick={() => exportToExcel('all')} variant="outline" className="gap-2">
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="w-[50px]">
                        <Checkbox
                          checked={selectedPermits.length === filteredPermits.length}
                  ...
                  <TableBody>
                    {filteredPermits.map((permit) => (
                      <TableRow 
                        key={permit.id}
                        className={selectedPermits.includes(permit.id) ? "bg-muted/50" : ""}
                      >
                        <TableCell>
                          <Checkbox
