...
const CallCenterReports = () => {
  ...
  const exportToExcel = () => {
    const excelData = filteredReports.map((report, index) => ({
      ...
      'البلاغ': report.report,
      'مشرف أعمار': report.constructionSupervisor
    }));

    const ws = XLSX.utils.json_to_sheet(excelData, {
      header: ['م', 'التوقيت', 'التاريخ', 'رقم الوحدة', 'اسم القطاع', 'مستلم البلاغ', 'البلاغ', 'مشرف أعمار']
    });
  ...
  const exportSelectedToExcel = () => {
    ...
    const selectedData = filteredReports
    ...
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'بلاغات الكول سنتر');
    XLSX.writeFile(wb, `بلاغات_الكول_سنتر_المحددة_${format(new Date(), 'yyyy-MM-dd')}.xlsx`);
  ...
  const exportToPDF = () => {
    ...
    autoTable(pdf, {
      startY: 60,
      head: [['م', 'التوقيت', 'التاريخ', 'رقم الوحدة', 'اسم القطاع', 'مستلم البلاغ', 'البلاغ', 'مشرف أعمار']],
      body: tableData,
      styles: {
        font: 'helvetica',
        fontSize: 10,
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-right">البلاغ</TableHead>
                  ...
                  <TableBody>
                    {filteredReports.map((report, index) => (
                      ...
                      >
                        ...
                        <TableCell className="text-right" style={{whiteSpace: 'pre-line', wordBreak: 'break-word', maxWidth: 250}}>{report.report}</TableCell>
